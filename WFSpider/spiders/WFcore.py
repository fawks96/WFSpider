...
class WfcoreSpider(CrawlSpider):
    ...
    def start_requests(self):
        ...
        find_res_list = list(find_res)[::-1]
        find_res_list = random.sample(find_res_list, len(find_res_list))
        for jn_idx in find_res_list:
    ...
    def parse_index(self, response):
        ...
        if not self.target_issues:
            ...
            find_current_location = response.xpath('//a[@class="yearSwitch"]/following-sibling::p[1]/a[@class="current"]')
            for location in find_current_location:
                ...
                if contains_target_year(loc_url):
                    ...
                    for next_uri in set(self.target_issues):
                        next_url = urljoin(base_url, next_uri)
                        if next_url in all_parsed_url:
                            logging.info("Jumped over parsed issue: %s" % next_url)
                            continue
